...
int main(){try{
 ...
 check(std::isnan(lab_signal(-1))&&std::isnan(lab_signal(29)),"invalid field rejected");
 lab_set(1,.4);for(int i=0;i<150;i++)lab_step(100);
 std::array<double,31> before{};for(int k=0;k<31;k++)before[k]=lab_get(k);
 for(int repeat=0;repeat<20;repeat++)for(int k=0;k<29;k++)check(std::isfinite(lab_signal(k)),"diagnostic finite");
 ...
 lab_reset(0);for(int i=0;i<100;i++){lab_step(100);for(int k=0;k<29;k++)(void)lab_signal(k);}
