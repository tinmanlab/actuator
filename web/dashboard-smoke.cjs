...
(async()=>{
 ...
 try{
  ...
  const gateAfter=await page.locator('#bridge-circuit .gate.on').evaluateAll(nodes=>nodes.map(n=>n.id).join(','));
  ok(gateBefore!==gateAfter,'auto replay advances visible inverter switch state');
